...
import servicesettingsFakerFactory from "./servicesettingsFakerFactory";
...
const ServicesettingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = servicesettingsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("servicesettings").create(elem));
...
export default connect(mapState, mapDispatch)(ServicesettingsFakerDialogComponent);
