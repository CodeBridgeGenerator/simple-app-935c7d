...
import seederData from "./servicesettings.seeder.json";
...
const ServicesettingsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("servicesettings").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ServicesettingsSeederDialogComponent);
